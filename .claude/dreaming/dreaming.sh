#!/bin/bash
# depl-orch project dreaming pass.
#
# Purpose: scheduled (weekly) curation of THIS project only — AGENTS.md/CLAUDE.md
# drift, stale memory, roadmap drift, the no-LLM-in-pipeline invariant, OpenCode
# model parity, CI/test gaps, recurring PR themes. Read-only — outputs a report.
# Scoped to depl-orch alone (never cross-mines other projects).
#
# Schedule: systemd user timer (Sun 07:00 nominal, Persistent=true catches up
# at next login).
...
echo "[$(date -Iseconds)] depl-orch dreaming pass started" >> "$LOG"
...
EXIT=$?
echo "[$(date -Iseconds)] depl-orch dreaming finished (exit=$EXIT, report=$REPORT)" >> "$LOG"
